...
export function createMarketHandler({authenticate,rpc,manifest,publicBase,fetchPublic=fetch,delay=ms=>new Promise(r=>setTimeout(r,ms)),computeParts=null}){
  return async request=>{
    ...
    try{
      ...
      if(begin.lease){
        lease=begin.lease;
        const report=computeParts?await computeParts({uid,lease,params}):await computeReportPart({uid,lease,params,manifest,publicBase,rpc,fetchPublic});
...
export async function computeReportPart({uid,lease,params,part=null,manifest,publicBase,rpc,fetchPublic=fetch}){
  ...
  for(const year of new Set([...selectedMonths].map(month=>month.slice(0,4)))){
    ...
    for(const row of rows)if(typeof row[0]!=='string'||row[0].slice(0,4)!==year||row[0]>'2025-08')throw Error('public_snapshot_mismatch');
    accumulator.addPublic(rows,selectedMonths);
  }
  for(let after=0;;){
    const page=await rpc('housing_market_private_rows',{p_uid:uid,p_lease:lease,p_after:after,p_page_size:5000,p_params:params});
    if(page.error)throw Error(page.error);accumulator.addPrivate(page.rows);if(!page.next_id)break;after=page.next_id;
  }
