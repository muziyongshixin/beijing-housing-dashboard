...
function combined(buckets,field){const values=new Float64Array(buckets.reduce((n,item)=>n+item[field].length,0));let offset=0;for(const item of buckets){values.set(item[field],offset);offset+=item[field].length;}return values;}
...
export function createMarketAccumulator(params,catalog){
  const benchmark=pair(params.metric),districts=new Map(),communities=new Map(),mapGroups=new Map(),trendScopes=new Map([['全部',new Map()]]);
  ...
  function add(month,district,businessArea,community,area,price,cycle,discount,rooms){
    ...
    if(!trendScopes.has(district))trendScopes.set(district,new Map());
