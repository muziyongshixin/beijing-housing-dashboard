...
(()=>{
  let worker,serial=0,failed=null;const pending=new Map();
  const fail=error=>{failed=error;for(const p of pending.values())p.reject(error);pending.clear();};
  try{worker=new Worker('./pages-worker.js?v=cea18d4e1bb26109');}catch(e){fail(new Error('后台计算无法启动，请使用现代浏览器并通过 HTTP(S) 打开页面。'));}
  if(worker){worker.onmessage=({data:r})=>{const p=pending.get(r.id);if(!p)return;if(r.progress!==undefined){p.progress?.(r.progress);return;}pending.delete(r.id);r.error?p.reject(new Error(r.error)):p.resolve(r.value);};worker.onerror=()=>fail(new Error('后台计算加载失败，请刷新页面重试。'));worker.onmessageerror=()=>fail(new Error('后台计算结果读取失败，请刷新页面重试。'));}
  ...
  window.DashboardData={initialize:progress=>call('initialize',null,progress)};
